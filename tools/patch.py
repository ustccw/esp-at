...
def main():
    ...
    for section in config.sections():
        ...
        if dependencies_option:
            with_sdkconfig = True if when == 'with_sdkconfig' else False
        ...
        if patch_name.endswith('.patch'):
            cmd = f'cd {dst_patch_path} && git apply --check {src_patch_path} && cd -'
            ret = subprocess.run(cmd, capture_output = True, shell = True)
            if ret.returncode:
                ESP_LOGI(f'{patch_name} does not need to be applied.')
            else:
                cmd = f'cd {dst_patch_path} && git apply {src_patch_path} && cd -'
